...
import { Copy, Check, Trash2, FileText, Zap, Download, AlertCircle, Clock, Square } from "lucide-react";
import { AISetupScreen } from "./ai-setup-screen";

const STYLES = [
  { id: "comprehensive", label: "Comprehensive / Production" },
  { id: "minimal", label: "Minimal / Clean" },
  { id: "cli", label: "CLI / Utility Tool" },
];

export const AIReadmeGenerator: React.FC = () => {
  const [prompt, setPrompt] = useState(
    "ToolVerse - Modern, privacy-first developer utility suite built with Next.js 15, React, TypeScript, and Neon DB."
  );
  ...
  useEffect(() => {
    const saved = localStorage.getItem("toolverse_ai_readme_generator");
    if (saved) {
      try {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        localStorage.setItem(
          "toolverse_ai_readme_generator",
          JSON.stringify({ prompt, output: data.result, style })
        );
        setHasHistory(true);
      } else {
        if (res.status === 401 || data.errorType === "API_KEY_REQUIRED") {
          ...
          setError(data.message || data.error || "Failed to generate README.");
        }
      }
    } catch (err: any) {
  ...
  const handleClear = () => {
    ...
    setOutput("");
    setError("");
    localStorage.removeItem("toolverse_ai_readme_generator");
    setHasHistory(false);
  };
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex justify-between items-center mt-1.5 text-[11px] text-zinc-400">
              ...
              <span>Valid GitHub Markdown Format</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            {loading ? (
              ...
              >
                <FileText className="w-4 h-4" /> Generate README.md
              </button>
            )}
