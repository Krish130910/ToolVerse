...
import { Sparkles, Copy, Check, Trash2, Palette, Zap } from "lucide-react";

const FORMATS = ["Tailwind CSS", "Vanilla CSS", "SCSS"];

export const AICssGenerator: React.FC = () => {
  const [prompt, setPrompt] = useState("Modern glassmorphism card with subtle orange hover glow, rounded corners, and soft backdrop blur");
  const [format, setFormat] = useState("Tailwind CSS");
  ...
  const handleGenerate = async () => {
    if (!prompt.trim()) {
      setError("Please describe the UI element styling.");
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.error || "Failed to generate CSS.");
      }
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between pb-4 border-b border-zinc-200/80">
        <span className="px-3 py-1 rounded-full bg-orange-500/10 text-orange-600 text-xs font-bold flex items-center gap-1.5 border border-orange-500/20">
          <Palette className="w-3.5 h-3.5" /> AI UI Styling &amp; CSS Generator
        </span>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-zinc-700 uppercase tracking-wider mb-2">
              Describe UI Element &amp; Style Expectations
            </label>
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Floating CTA button with gradient border and subtle bounce animation..."
              rows={8}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {loading ? (
                <>
                  <Zap className="w-4 h-4 animate-spin" /> Generating CSS...
                </>
              ) : (
                <>
                  <Palette className="w-4 h-4" /> Generate {format}
                </>
              )}
            </button>
            <button
          ...
          <div className="w-full h-[360px] rounded-xl bg-[#121215] border border-zinc-800 p-4 text-zinc-200 font-mono text-xs overflow-auto shadow-inner">
            {output ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-zinc-500 space-y-2 text-center">
                ...
                <p>Describe UI component layout to generate CSS or Tailwind classes.</p>
              </div>
            )}
          </div>
        </div>
